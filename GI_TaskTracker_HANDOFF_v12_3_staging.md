# GI Task Tracker — Developer Handoff v8
*Continuation from previous session — includes v12.3 staging-only nested subtasks, prior v12.2 mobile/touch action-sheet fix, confirmed production/staging split, and release-promotion rules*
...
**Latest generated staging build:** `v12.3 — STAGING — tasktracker_staging/state` / nested subtasks under main tasks  
**Latest live build:** `v12.1 — LIVE — tasktracker/state` / hidden admin-only Backup / Restore controls promoted to live  
**Previous staging build:** `v12.2 — STAGING — tasktracker_staging/state` / mobile action-sheet context fix + conservative mobile/touch detection  
...
### Current staging build target

```javascript
var APP_VERSION = 'v12.3 — STAGING — tasktracker_staging/state';
var DATA_NODE   = 'tasktracker_staging/state';
var STREAM_NODE = 'tasktracker_staging/state';
```

No live/production build was generated for v12.2 or v12.3 unless Kyle explicitly promotes a staged version later.

---

## v12.3 Staging Change — Nested Subtasks

### Purpose
Kyle requested the ability to create a “subtask” or nested task under a main task, with the same core functions as normal tasks including dates and status.

### Implemented scope
This build intentionally implements **one-level subtasks** under a parent task. It does **not** implement infinite nesting or sub-subtasks. This keeps the change surgical and avoids broad schema/rendering risk.

### Behavior added
- Main task rows now include a `+ Subtask` action.
- On mobile/tablet action sheet, main tasks now include `Add Subtask`.
- Subtasks are stored inside the parent task as a `subtasks` array.
- Subtasks use the same editable field set as main tasks:
  - Customer / title
  - Details / notes
  - Owner
  - Supporting
  ...
  - Next Milestone
  - Meeting Notes
- Subtasks can be edited, deleted, and viewed in history/change log.
- Subtasks render visually underneath the parent row with indentation and a `Subtask` label.
- Parent task rows show a small progress summary when subtasks exist, such as `1/3 subtasks complete`.
- Dashboard and My Tasks include subtasks in their result sets.
- CSV exports include subtasks, labeled as `Subtask of:` in the bucket/context field.

### Functions/areas touched
- CSS around `.row-acts` plus new subtask display classes.
- Mobile action sheet markup and functions.
- Task rendering in `renderApp()`.
- Task form flow:
  - `openAddTask()`
  - `openAddSubtask()`
  - `openEditTask()`
...
- Task helpers:
  ...
  - `visibleSubtasksFor()`
  - `subtaskSummary()`
- Dashboard / My Tasks / CSV collection paths:
  - `getAllTasks()`
  - `renderMyTasks()` / `updateMyTasksTable()`
  - `refreshDashTable()`
  - `exportCSV()`
  - `exportAllCSV()`
- Sync/merge helper indexing:
  - `mergeStates()` → `buildIndex()` now indexes subtasks by ID and can pull in new remote subtasks under the correct parent when possible.

### Intentional limitations / cautions
- Subtasks are one level deep only.
- Subtasks are saved as nested data under the parent task, so the schema is extended but not rebuilt.
- Existing tasks without `subtasks` remain valid; helper code initializes missing `subtasks` arrays as needed.
- Deleted subtasks follow the app’s existing general merge limitations around deletions; this change improves subtask add/edit merge awareness but does not fully redesign deletion conflict handling.
- Production/live remains untouched until Kyle explicitly requests promotion.

### Test checklist
- Add a subtask under a main task.
- Edit a subtask and confirm all same fields are available.
- Change subtask due date/status/priority and verify display updates.
- Delete a subtask and verify parent task remains.
- Confirm main task add/edit/delete still works.
- Confirm mobile action sheet shows Add Subtask for main tasks and hides it for subtasks.
- Confirm Dashboard and My Tasks show subtasks.
- Confirm CSV export includes subtasks with a `Subtask of:` label.
- Confirm visible environment label reads `v12.3 — STAGING — tasktracker_staging/state`.
...
**Latest generated staging build:** `v12.2` / staging-only mobile action sheet + Windows touch-detection fix
