...
## v12.3 — 2026-05-09
**Nested Subtasks — STAGING ONLY**

This staging release adds one-level subtasks under normal tasks. No live/production file was generated.

### Changed
- Bumped `APP_VERSION` to `v12.3 — STAGING — tasktracker_staging/state`.
- Kept both Firebase nodes pointed to staging:
...
### Added
- Added a `+ Subtask` action on each main task row.
- Added a mobile action-sheet option to add a subtask from a main task.
- Added one-level nested `subtasks` arrays under parent tasks.
- Subtasks use the same task form as normal tasks, including:
  - customer/title
  - details/notes
  - owner
  - supporting person
  ...
  - milestone
  - meeting notes
- Subtasks can be edited, deleted, and viewed in the change log/history just like normal tasks.
- Subtasks display visually underneath their parent task with a `Subtask` label and indentation.
- Parent tasks show a small subtask completion summary such as `1/3 subtasks complete`.
- Dashboard, My Tasks, and CSV exports now include subtasks, labeled as subtasks / subtask-of where relevant.
- Merge/sync indexing now includes subtask IDs so edits to subtasks can be merged more safely instead of only treating the parent task as the merge unit.

### Not changed
- No production/live build was created.
- No Firebase project URL/API key changes.
- No Firebase wrapped data format changes.
- No change to production/live `tasktracker/state`.
- No multi-level infinite nesting; subtasks are intentionally one level deep under a main task.
- No broad refactor of the task schema or rendering system.

### Test notes
- On staging, add a subtask under a normal task and confirm it appears directly below the parent.
- Edit the subtask and confirm the same task form opens with the subtask values.
- Change subtask due date/status/priority and confirm the row updates correctly.
- Use mobile/touch layout and confirm tapping a main task shows `Add Subtask`, while tapping a subtask hides that option.
- Confirm My Tasks and Dashboard include the subtask.
- Confirm CSV exports include subtasks with a `Subtask of:` bucket label.
- Confirm normal main task add/edit/delete behavior still works.
