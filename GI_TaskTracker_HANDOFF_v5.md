# GI Task Tracker — Developer Handoff v5
*Continuation from previous session — includes confirmed v11.6 recovery, surgical future-development workflow, and production/staging Firebase guidance*
...
| Environment | Hosted where | Firebase target | Purpose |
|---|---|---|---|
| Production | Company website | `tasktracker/state` | Stable version used by other GI users |
| Staging | GitHub Pages | `tasktracker_staging/state` | Testing new versions before promotion |
...
4. Upload staging build to GitHub Pages and test there.
5. Test add/edit/delete/refresh and, if relevant, multi-PC merge behavior.
6. After staging is approved, create or update a production build that changes only the Firebase target back to `tasktracker/state`.
7. Upload the production build to the company website.
8. Update the changelog and handoff with what was promoted.
...
6. Never silently change `FB_DB_URL`, `FB_PATH`, `fbRead()`, `fbWrite()`, `serializeState()`, `persistNow()`, or the wrapped Firebase format.
